...
class BedrockAgentAdapter(ModelAdapter):
    ...
    def get_file_from_s3(
        self, file_info: Dict[str, str], use_s3_path: bool = False
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if use_s3_path:
                return {
                    ...
                    "source": {
                        "s3Path": (
                            f"s3://{os.environ['CHATBOT_FILES_BUCKET_NAME']}"
                            f"/{s3_path}"
                        )
                    },
                }
    ...
    def _prepare_file_prompt(
        ...
        if images:
            image_info = [
                {k: v for k, v in img.items() if k != "source"} for img in images
            ]
            logger.info(f"Raw image information: {json.dumps(image_info)}")
        ...
        if documents:
            doc_info = [
                {k: v for k, v in doc.items() if k != "source"} for doc in documents
            ]
            logger.info(f"Raw document information: {json.dumps(doc_info)}")
        ...
        enhanced_prompt += (
            "The following files have been uploaded and are available "
            "for processing:\n\n"
        )
        ...
        if images:
            ...
            for i, image in enumerate(images):
                ...
                s3_uri = (
                    f"s3://{os.environ['CHATBOT_FILES_BUCKET_NAME']}"
                    f"/private/{self.user_id}/{file_key}"
                )
                enhanced_prompt += (
                    f"{i+1}. Filename: {file_key}\n   S3 URI: {s3_uri}\n"
                    f"   Format: {image.get('format', 'jpg')}\n\n"
                )
        ...
        if images:
            ...
            for i, image in enumerate(images):
                if "source" in image and "bytes" in image["source"]:
                    logger.info(
                        f"Image {i} format: {image.get('format', 'unknown')}, "
                        f"size: {len(image['source']['bytes'])} bytes"
                    )
                else:
        ...
        if documents:
            ...
            for i, doc in enumerate(documents):
                if "source" in doc and "bytes" in doc["source"]:
                    logger.info(
                        f"Document {i} format: {doc.get('format', 'unknown')}, "
                        f"size: {len(doc['source']['bytes'])} bytes"
                    )
                else:
        ...
        enhanced_prompt += "\n\n### REMINDER ###"
        enhanced_prompt += (
            "\nYou can use the S3 URIs provided at the beginning of this "
            "message with your /get-file endpoint to retrieve and process "
            "the files."
        )

        # The agent will use the /get-file endpoint to retrieve files, so we
        # don't need to provide API schema information here.
        # The agent will need to know the S3 bucket name and the user's
        # private directory.
    ...
    def _create_system_prompt(self, has_files=False) -> str:
        ...
        """
        system_prompt = (
            "You are a helpful AI assistant with access to various APIs "
        ...
        if has_files:
            system_prompt += (
                "\n\nWhen files are uploaded, you will receive their S3 URIs "
                "in the format:"
            )
            system_prompt += (
                f"\ns3://{os.environ['CHATBOT_FILES_BUCKET_NAME']}"
                f"/private/{self.user_id}/<filename>"
            )
            system_prompt += (
                "\n\nYou can use these URIs with your /get-file endpoint "
                "to retrieve and process the files."
            )
    ...
    def run(
        ...
        try:
            ...
            enhanced_prompt = self._prepare_file_prompt(
                prompt, processed_images, processed_documents
            )
            ...
            if agent_id:
                ...
                response = invoke_agent_by_id(
                    ...
                    enable_trace=True,
                    timeout=30,  # Use shorter timeout to avoid Lambda timeouts
                )
            else:
            ...
            # Convert any float values in the trace to Decimal for DynamoDB
            # compatibility
            ...
            try:
                # Convert all floats to Decimal and handle binary data for
            ...
            if user_groups and (
                "admin" in user_groups or "workspace_manager" in user_groups
            ):
                # We need to manually add the metadata to avoid float issues
                try:
                    # Get the latest messages again (including our newly added
                    # AI message)
                    messages = messages_to_dict(
                        self.chat_history.get_messages_from_storage()
                    )
                    if messages:
                        ...
                        # Save to DynamoDB with proper float handling
                        messages_json = json.dumps(messages, cls=CustomJSONEncoder)
                        messages_with_decimal = json.loads(
                            messages_json, parse_float=Decimal
                        )
            ...
            response = {
                ...
                "metadata": (
                    metadata
                    if user_groups
                    and ("admin" in user_groups or "workspace_manager" in user_groups)
                    else {
                        "sessionId": self.session_id,
                    }
                ),
